...
species_richness_bar_plot <- all_species_richness |>
  ...
  ggplot(aes(x = year,
             y = spp_rich,
             fill = type)) +
  # layer 1: bars for species richness 
  geom_col() +
  # facet by transect
...
# combine native and non-native richness data
all_species_richness <- bind_rows(
  #adding species type column to native and invasive spp richness
  native_species_rich |>
    mutate(type = "Native"),
  invasive_species_rich |>
    mutate(type = "Non-native")
...
species_richness_summary <- all_species_richness |>
  #group by year and spp type
  group_by(year, type) |>
  #calculate total species richness for each year and type
  summarize(
...
species_richness_bar_plot <- species_richness_summary |>
  # base layer: ggplot
  # x axis is year
  # y axis is total_richness
  # fill by spp type
  ggplot(aes(x = year,
             y = total_richness,
             fill = type)) +
  # first layer: stacked bars
  geom_col() +
  # second layer: line showing prescribed fire
  # customizing line
  geom_vline(
    xintercept = 2023.5,
    linetype = "dashed",
  ...
  # second layer: label prescribed fire
  annotate(
    "text",
    x = 2023.5,
    y = 184,
...
selected_species_richness_bar_plot <- selected_species_richness_summary |>
  ...
  ggplot(aes(x = year,
             y = total_richness,
             fill = species_group)) +
  # first layer: create stacked bars
  geom_col() +
  # second layer: prescribed fire line
  # customizing line
  geom_vline(
    xintercept = 2023.5,
    linetype = "dashed",
  ...
  # third layer: label prescribed fire event
  # customizing label
  annotate(
    "text",
    x = 2023.5,
  ...
  # add labels for x/y axis, title, legend
  labs(
  ...
  # remove gap at the bottom, set y-axis limits
