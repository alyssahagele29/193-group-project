...
```{r}
#| label: alyssa vis-3 species richness invasive boxplot
...
```{r}
#| label: alyssa vis-5 density plot focus native/non native richness
...
selected_species_richness <- selected_species_cover |>
  ...
  summarize(spp_rich = n_distinct(psoc),
            .groups = "drop")
# create density plot
selected_species_richness_density <- selected_species_richness |>
  # create ggplot
  ggplot(aes(x = spp_rich,
             fill = species_group,
             color = species_group)) +
  ...
  # add labels
  labs(
    title = "Selected Species Richness Distributions",
    x = "Species Richness",
    y = "Density",
    fill = "Species Group",
    color = "Species Group"
  ...
  theme_bw()
# display plot
selected_species_richness_density
```

>>>>>>> 556db92eb658f3daa46cc7fa5243c0d7e2a7bbe8
